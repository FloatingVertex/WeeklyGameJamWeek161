...
namespace PolyNav
{
    ...
    public class PolyNav2D : MonoBehaviour
    {
        ...
        public void GenerateMap(bool generateMaster) {
            ...
            Debug.Log("Generating navmesh took: " + timer.ElapsedMilliseconds + "ms");
        }
        ...
        void CreatePolyMap(bool generateMaster) {
            ...
            if (GetComponent<TerrainManager>() && GetComponent<TerrainManager>().playModeTransformToFollow)
            {
                masterPolys = new List<Polygon>();
                var center = (Vector2)GetComponent<TerrainManager>().playModeTransformToFollow.position;
                var radius = GetComponent<TerrainManager>().playModeViewRange / 1.5f;
                var tlMaster = center + new Vector2(-radius, radius);
                var trMaster = center + new Vector2(radius, radius);
                var brMaster = center + new Vector2(radius, -radius);
                var blMaster = center + new Vector2(-radius, -radius);
                var pointsMaster = new Vector2[] { tlMaster, blMaster, brMaster, trMaster };
                var transformedMaster = TransformPoints(ref pointsMaster, masterCollider.transform);
                var inflatedMaster = InflatePolygon(ref transformedMaster, Mathf.Max(0.01f, radiusOffset));
                masterPolys.Add(new Polygon(inflatedMaster));
            }
